 ...
 * Validation
 */
var plugins={rdMailForm:$(".rd-mailform"),regula:$("[data-constraints]")},rdMailform={MF000:"Successfully sent!",MF001:"Recipients are not set!",MF002:"Form will not work locally!",MF003:"Please, define email field in your form!",MF004:"Please, define type of your form!",MF254:"Something went wrong with PHPMailer!",MF255:"Aw, snap! Something went wrong."};$(document).ready(function(){function a(a){for(var b=0;b<a.length;b++){var d,c=$(a[b]);c.addClass("form-control-has-validation").after("<span class='form-validation'></span>"),d=c.parent().find(".form-validation"),d.is(":last-child")&&c.addClass("form-control-last-child")}a.on("input change propertychange blur",function(a){var d,c=$(this);if(("blur"==a.type||c.parent().hasClass("has-error"))&&!c.parents(".rd-mailform").hasClass("success"))if((d=c.regula("validate")).length)for(b=0;b<d.length;b++)c.siblings(".form-validation").text(d[b].message).parent().addClass("has-error");else c.siblings(".form-validation").text("").parent().removeClass("has-error")}).regula("bind")}function b(a){var b,c=0;if(a.length){for(d=0;d<a.length;d++){var f=$(a[d]);if((b=f.regula("validate")).length)for(e=0;e<b.length;e++)c++,f.siblings(".form-validation").text(b[e].message).parent().addClass("has-error");else f.siblings(".form-validation").text("").parent().removeClass("has-error")}return 0==c}return!0}if(plugins.regula.length&&a(plugins.regula),plugins.rdMailForm.length){var c,d,e;for(c=0;c<plugins.rdMailForm.length;c++){var g=$(plugins.rdMailForm[c]);g.attr("novalidate","novalidate").ajaxForm({data:{"form-type":g.attr("data-form-type")||"contact",counter:c},beforeSubmit:function(){var a=$(plugins.rdMailForm[this.extraData.counter]),c=a.find("[data-constraints]");if(!b(c))return!1;var d=$("#"+a.attr("data-form-output"));d.hasClass("snackbars")&&(d.html('<p><span class="icon text-middle fa fa-circle-o-notch fa-spin icon-xxs"></span><span>Sending</span></p>'),d.addClass("active"))},error:function(a){var b=$("#"+$(plugins.rdMailForm[this.extraData.counter]).attr("data-form-output"));b.text(rdMailform[a])},success:function(a){var b=$(plugins.rdMailForm[this.extraData.counter]),c=$("#"+b.attr("data-form-output")),e=b.find("select");if(e.length)for(d=0;d<e.length;d++){var g=$(e[d]);g.select2("val",null)}b.addClass("success"),a=5==a.length?a:"MF255",c.text(rdMailform[a]),"MF000"===a?(c.hasClass("snackbars")&&c.html('<p><span class="icon text-middle mdi mdi-check icon-xxs"></span><span>'+rdMailform[a]+"</span></p>"),c.addClass("success active")):(c.hasClass("snackbars")&&c.html(' <p class="snackbars-left"><span class="icon icon-xxs mdi mdi-alert-outline text-middle"></span><span>'+rdMailform[a]+"</span></p>"),c.addClass("error active")),b.clearForm(),b.find("input, textarea").blur(),setTimeout(function(){c.removeClass("active error success"),b.removeClass("success")},5e3)}})}}});